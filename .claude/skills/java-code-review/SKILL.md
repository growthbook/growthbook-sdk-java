...
- `List.of()` / `Set.of()` / `Map.of()` / `List.copyOf()` — Java 9+, not available here; require `Arrays.asList` + `Collections.unmodifiableList` etc.
- Parallel streams without understanding implications

**Suggest:**
- `Collections.unmodifiableList(new ArrayList<>(source))` for defensive copies (Java 8)
